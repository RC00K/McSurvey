// Import the images
import dtep1 from '../images/DTEP1.jpg';
import dtep2 from '../images/DTEP2.jpg';
import dtep3 from '../images/DTEP3.jpg';
import dtep4 from '../images/DTEP4.jpg';
import dtep5 from '../images/DTEP5.jpg';
import dtep6 from '../images/DTEP6.jpg';

import sp1 from '../images/SP1.jpg';
import sp2 from '../images/SP2.jpg';
import sp3 from '../images/SP3.jpg';
import sp4 from '../images/SP4.jpg';
import sp5 from '../images/SP5.jpg';

import ns1 from '../images/NS1.jpg';
import ns2 from '../images/NS2.jpg';
import ns3 from '../images/NS3.jpg';
import ns4 from '../images/NS4.jpg';
import ns5 from '../images/NS5.jpg';
import ns6 from '../images/NS6.jpg';

import mc1 from '../images/MC1.jpg';
import mc2 from '../images/MC2.jpg';
import mc3 from '../images/MC3.jpg';
import mc4 from '../images/MC4.jpg';
import mc5 from '../images/MC5.jpg';
import mc6 from '../images/MC6.jpg';
import mc7 from '../images/MC7.jpg';

import bps1 from '../images/BPS1.jpg';
import bps2 from '../images/BPS2.jpg';
import bps3 from '../images/BPS3.jpg';
import bps4 from '../images/BPS4.jpg';
import bps5 from '../images/BPS5.jpg';
import bps6 from '../images/BPS6.jpg';

import wa1 from '../images/WA1.jpg';
import wa2 from '../images/WA2.jpg';
import wa3 from '../images/WA3.jpg';


export const oneDrive = [
    {
        projectType: "AOT Greens Fees",
        projectTitle: "AOTGF - 1 Drive Thru",
        storeNumber: "",
        questions: [
            {
                questionTitle: "Drive-Thru Entry Point",
                questionDesc: "Each drive lane & gateway signage",
                question: "Take a picture showing the drive lane entry and surrounding gateway with order point.",
                questionImages: [
                    {
                        imageTitle: "Drive-Thru Example 1",
                        imageAlt: "Drive-Thru Entry Point 1",
                        image: dtep1
                    },
                    {
                        imageTitle: "Drive-Thru Example 2",
                        imageAlt: "Drive-Thru Entry Point 2",
                        image: dtep2
                    },
                    {
                        imageTitle: "Drive-Thru Example 3",
                        imageAlt: "Drive-Thru Entry Point 3",
                        image: dtep3
                    },
                    {
                        imageTitle: "Drive-Thru Example 4",
                        imageAlt: "Drive-Thru Entry Point 4",
                        image: dtep4
                    },
                    {
                        imageTitle: "Drive-Thru Example 5",
                        imageAlt: "Drive-Thru Entry Point 5",
                        image: dtep5
                    },
                    {
                        imageTitle: "Drive-Thru Example 6",
                        imageAlt: "Drive-Thru Entry Point 6",
                        image: dtep6
                    }
                ]
            },
            {
                questionTitle: "Drive-Thru",
                questionDesc: "Speaker post & menu board",
                question: "Take a picture of the speaker post and menu board.",
                questionImages: [
                    {
                        imageTitle: "Speaker Post Example 1",
                        imageAlt: "Speaker Post 1",
                        image: sp1
                    },
                    {
                        imageTitle: "Speaker Post Example 2",
                        imageAlt: "Speaker Post 2",
                        image: sp2
                    },
                    {
                        imageTitle: "Speaker Post Example 3",
                        imageAlt: "Speaker Post 3",
                        image: sp3
                    },
                    {
                        imageTitle: "Speaker Post Example 4",
                        imageAlt: "Speaker Post 4",
                        image: sp4
                    },
                    {
                        imageTitle: "Speaker Post Example 5",
                        imageAlt: "Speaker Post 5",
                        image: sp5
                    }
                ]
            },
            {
                questionTitle: "Network Switches & Patch Panel",
                questionDesc: "Tech rack with switches & patch panel",
                question: "Take a picture showing the entire network rack to include switches and wiring layout.",
                questionImages: [
                    {
                        imageTitle: "Network Switches Example 1",
                        imageAlt: "Network Switches 1",
                        image: ns1
                    },
                    {
                        imageTitle: "Network Switches Example 2",
                        imageAlt: "Network Switches 2",
                        image: ns2
                    },
                    {
                        imageTitle: "Network Switches Example 3",
                        imageAlt: "Network Switches 3",
                        image: ns3
                    },
                    {
                        imageTitle: "Network Switches Example 4",
                        imageAlt: "Network Switches 4",
                        image: ns4
                    },
                    {
                        imageTitle: "Network Switches Example 5",
                        imageAlt: "Network Switches 5",
                        image: ns5
                    },
                    {
                        imageTitle: "Network Switches Example 6",
                        imageAlt: "Network Switches 6",
                        image: ns6
                    }
                ]
            },
            {
                questionTitle: "Modem Connectivity",
                questionDesc: "Placement & connected cables",
                question: "Take a picture showing the aruba network controller port 3 cable connected to the back of the modem.",
                questionHints: [
                    {
                        hint: "Aruba controller is likely the bottom switch on the rack."
                    },
                    {
                        hint: "Locate port 3 cable."
                    },
                    {
                        hint: "Trace the cable to the modem."
                    }
                ],
                questionImages: [
                    {
                        imageTitle: "Modem Connectivity Example 1",
                        imageAlt: "Modem Connectivity 1",
                        image: mc1
                    },
                    {
                        imageTitle: "Modem Connectivity Example 2",
                        imageAlt: "Modem Connectivity 2",
                        image: mc2
                    },
                    {
                        imageTitle: "Modem Connectivity Example 3",
                        imageAlt: "Modem Connectivity 3",
                        image: mc3
                    },
                    {
                        imageTitle: "Modem Connectivity Example 4",
                        imageAlt: "Modem Connectivity 4",
                        image: mc4
                    },
                    {
                        imageTitle: "Modem Connectivity Example 5",
                        imageAlt: "Modem Connectivity 5",
                        image: mc5
                    },
                    {
                        imageTitle: "Modem Connectivity Example 6",
                        imageAlt: "Modem Connectivity 6",
                        image: mc6
                    },
                    {
                        imageTitle: "Modem Connectivity Example 7",
                        imageAlt: "Modem Connectivity 7",
                        image: mc7
                    }
                ]
            },
            {
                questionTitle: "BOS & POS Servers",
                questionDesc: "Office servers & location",
                question: "Take a picture showing both the BOS and POS servers.",
                questionImages: [
                    {
                        imageTitle: "BOS & POS Servers Example 1",
                        imageAlt: "BOS & POS Servers 1",
                        image: bps1
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 2",
                        imageAlt: "BOS & POS Servers 2",
                        image: bps2
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 3",
                        imageAlt: "BOS & POS Servers 3",
                        image: bps3
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 4",
                        imageAlt: "BOS & POS Servers 4",
                        image: bps4
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 5",
                        imageAlt: "BOS & POS Servers 5",
                        image: bps5
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 6",
                        imageAlt: "BOS & POS Servers 6",
                        image: bps6
                    }
                ]
            },
            {
                questionTitle: "Tech Equipment Overview",
                questionDesc: "Full view of network environment",
                question: "Take a picture showing the office or POS equipment area showing the Network switches and patch panel with the BOS & POS servers.",
                questionImages: [
                    {
                        imageTitle: "Tech Equipment Overview Example 1",
                        imageAlt: "Tech Equipment Overview 1",
                        image: wa1
                    },
                    {
                        imageTitle: "Tech Equipment Overview Example 2",
                        imageAlt: "Tech Equipment Overview 2",
                        image: wa2
                    },
                    {
                        imageTitle: "Tech Equipment Overview Example 3",
                        imageAlt: "Tech Equipment Overview 3",
                        image: wa3
                    }
                ]
            }
        ]
    }
];

export const twoDrive = [
    {
        projectType: "AOT Greens Fees",
        projectTitle: "AOTGF - 2 Drive Thrus",
        storeNumber: "",
        questions: [
            {
                questionTitle: "Drive-Thru Entry Point",
                questionDesc: "Each drive lane & gateway signage",
                question: "Take a picture showing the drive lane entry and surrounding gateway with order point.",
                questionImages: [
                    {
                        imageTitle: "Drive-Thru Example 1",
                        imageAlt: "Drive-Thru Entry Point 1",
                        image: dtep1
                    },
                    {
                        imageTitle: "Drive-Thru Example 2",
                        imageAlt: "Drive-Thru Entry Point 2",
                        image: dtep2
                    },
                    {
                        imageTitle: "Drive-Thru Example 3",
                        imageAlt: "Drive-Thru Entry Point 3",
                        image: dtep3
                    },
                    {
                        imageTitle: "Drive-Thru Example 4",
                        imageAlt: "Drive-Thru Entry Point 4",
                        image: dtep4
                    },
                    {
                        imageTitle: "Drive-Thru Example 5",
                        imageAlt: "Drive-Thru Entry Point 5",
                        image: dtep5
                    },
                    {
                        imageTitle: "Drive-Thru Example 6",
                        imageAlt: "Drive-Thru Entry Point 6",
                        image: dtep6
                    }
                ]
            },
            {
                questionTitle: "Drive-Thru 1",
                questionDesc: "Speaker post & menu board",
                question: "Take a picture of the speaker post and menu board.",
                questionImages: [
                    {
                        imageTitle: "Speaker Post Example 1",
                        imageAlt: "Speaker Post 1",
                        image: sp1
                    },
                    {
                        imageTitle: "Speaker Post Example 2",
                        imageAlt: "Speaker Post 2",
                        image: sp2
                    },
                    {
                        imageTitle: "Speaker Post Example 3",
                        imageAlt: "Speaker Post 3",
                        image: sp3
                    },
                    {
                        imageTitle: "Speaker Post Example 4",
                        imageAlt: "Speaker Post 4",
                        image: sp4
                    },
                    {
                        imageTitle: "Speaker Post Example 5",
                        imageAlt: "Speaker Post 5",
                        image: sp5
                    }
                ]
            },
            {
                questionTitle: "Drive-Thru 2",
                questionDesc: "Speaker post & menu board",
                question: "Take a picture of the speaker post and menu board.",
                questionImages: [
                    {
                        imageTitle: "Speaker Post Example 1",
                        imageAlt: "Speaker Post 1",
                        image: sp1
                    },
                    {
                        imageTitle: "Speaker Post Example 2",
                        imageAlt: "Speaker Post 2",
                        image: sp2
                    },
                    {
                        imageTitle: "Speaker Post Example 3",
                        imageAlt: "Speaker Post 3",
                        image: sp3
                    },
                    {
                        imageTitle: "Speaker Post Example 4",
                        imageAlt: "Speaker Post 4",
                        image: sp4
                    },
                    {
                        imageTitle: "Speaker Post Example 5",
                        imageAlt: "Speaker Post 5",
                        image: sp5
                    }
                ]
            },
            {
                questionTitle: "Network Switches & Patch Panel",
                questionDesc: "Tech rack with switches & patch panel",
                question: "Take a picture showing the entire network rack to include switches and wiring layout.",
                questionImages: [
                    {
                        imageTitle: "Network Switches Example 1",
                        imageAlt: "Network Switches 1",
                        image: ns1
                    },
                    {
                        imageTitle: "Network Switches Example 2",
                        imageAlt: "Network Switches 2",
                        image: ns2
                    },
                    {
                        imageTitle: "Network Switches Example 3",
                        imageAlt: "Network Switches 3",
                        image: ns3
                    },
                    {
                        imageTitle: "Network Switches Example 4",
                        imageAlt: "Network Switches 4",
                        image: ns4
                    },
                    {
                        imageTitle: "Network Switches Example 5",
                        imageAlt: "Network Switches 5",
                        image: ns5
                    },
                    {
                        imageTitle: "Network Switches Example 6",
                        imageAlt: "Network Switches 6",
                        image: ns6
                    }
                ]
            },
            {
                questionTitle: "Modem Connectivity",
                questionDesc: "Placement & connected cables",
                question: "Take a picture showing the aruba network controller port 3 cable connected to the back of the modem.",
                questionHints: [
                    {
                        hint: "Aruba controller is likely the bottom switch on the rack."
                    },
                    {
                        hint: "Locate port 3 cable."
                    },
                    {
                        hint: "Trace the cable to the modem."
                    }
                ],
                questionImages: [
                    {
                        imageTitle: "Modem Connectivity Example 1",
                        imageAlt: "Modem Connectivity 1",
                        image: mc1
                    },
                    {
                        imageTitle: "Modem Connectivity Example 2",
                        imageAlt: "Modem Connectivity 2",
                        image: mc2
                    },
                    {
                        imageTitle: "Modem Connectivity Example 3",
                        imageAlt: "Modem Connectivity 3",
                        image: mc3
                    },
                    {
                        imageTitle: "Modem Connectivity Example 4",
                        imageAlt: "Modem Connectivity 4",
                        image: mc4
                    },
                    {
                        imageTitle: "Modem Connectivity Example 5",
                        imageAlt: "Modem Connectivity 5",
                        image: mc5
                    },
                    {
                        imageTitle: "Modem Connectivity Example 6",
                        imageAlt: "Modem Connectivity 6",
                        image: mc6
                    },
                    {
                        imageTitle: "Modem Connectivity Example 7",
                        imageAlt: "Modem Connectivity 7",
                        image: mc7
                    }
                ]
            },
            {
                questionTitle: "BOS & POS Servers",
                questionDesc: "Office servers & location",
                question: "Take a picture showing both the BOS and POS servers.",
                questionImages: [
                    {
                        imageTitle: "BOS & POS Servers Example 1",
                        imageAlt: "BOS & POS Servers 1",
                        image: bps1
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 2",
                        imageAlt: "BOS & POS Servers 2",
                        image: bps2
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 3",
                        imageAlt: "BOS & POS Servers 3",
                        image: bps3
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 4",
                        imageAlt: "BOS & POS Servers 4",
                        image: bps4
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 5",
                        imageAlt: "BOS & POS Servers 5",
                        image: bps5
                    },
                    {
                        imageTitle: "BOS & POS Servers Example 6",
                        imageAlt: "BOS & POS Servers 6",
                        image: bps6
                    }
                ]
            },
            {
                questionTitle: "Tech Equipment Overview",
                questionDesc: "Full view of network environment",
                question: "Take a picture showing the office or POS equipment area showing the Network switches and patch panel with the BOS & POS servers.",
                questionImages: [
                    {
                        imageTitle: "Tech Equipment Overview Example 1",
                        imageAlt: "Tech Equipment Overview 1",
                        image: wa1
                    },
                    {
                        imageTitle: "Tech Equipment Overview Example 2",
                        imageAlt: "Tech Equipment Overview 2",
                        image: wa2
                    },
                    {
                        imageTitle: "Tech Equipment Overview Example 3",
                        imageAlt: "Tech Equipment Overview 3",
                        image: wa3
                    }
                ]
            }
        ]
    } 
];